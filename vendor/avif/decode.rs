...
impl Decoder {
    ...
    fn from_parsed(avif: avif_parse::AvifData) -> Result<Self> {
        ...
        let alpha = avif
            .alpha_item
            .as_ref()
            .map(|a| Self::to_alpha(decoder.decode_frame(a)?))
            .transpose()?;
        ...
        let color =
            unsafe { std::mem::transmute::<FrameTempRef<'_>, FrameTempRef<'static>>(color) };
        Ok(Self {
            _decoder: decoder,
            color,
            alpha,
            ...
            let mut image = match (color, alpha) {
                ...
                (Image::Rgb8(img), AlphaImage::Gray16(alpha)) => {
                    let buf = img
                        .pixels()
                        .zip(alpha.pixels())
                        .map(|(c, a)| c.map(|c| (u16::from(c) << 8) | u16::from(c)).with_alpha(*a))
                        .collect();
                ...
                (Image::Rgb16(img), AlphaImage::Gray8(alpha)) => {
                    let buf = img
                        .pixels()
                        .zip(alpha.pixels())
                        .map(|(c, a)| c.with_alpha(u16::from(*a) << 8 | u16::from(*a)))
                        .collect();
                    Image::Rgba16(ImgVec::new(buf, img.width(), img.height()))
                }
                ...
                (Image::Gray16(img), AlphaImage::Gray8(alpha)) => {
                    let buf = img
                        .pixels()
                        .zip(alpha.pixels())
                        .map(|(c, a)| Rgba::new(*c, *c, *c, u16::from(*a) << 8 | u16::from(*a)))
                        .collect();
                    Image::Rgba16(ImgVec::new(buf, img.width(), img.height()))
                }
                    ...
                    Image::Rgba8(img) => {
                        img.pixels_mut().filter(|px| px.a > 0).for_each(|px| {
                            ...
                            fn unprem(val: u8, alpha: u8) -> u8 {
                                ((u16::from(val) * 256) / (u16::from(alpha) * 256) / 256).min(255)
                                    as u8
                            }
                    ...
                    Image::Rgba16(img) => {
                        img.pixels_mut().filter(|px| px.a > 0).for_each(|px| {
                            ...
                            fn unprem(val: u16, alpha: u16) -> u16 {
                                ((u32::from(val) * 0xFFFF) / (u32::from(alpha) * 0xFFFF) / 0xFFFF)
                                    .min(65535) as u16
                            }
                            px.r = unprem(px.r, px.a);
                            px.g = unprem(px.g, px.a);
        ...
        Ok(match img.rows_iter()? {
            RowsIters::YuvPlanes8 {
                ...
                let px_iter: &mut dyn Iterator<Item = YUV<u8>> = match chroma_sampling {
                    ...
                    color::ChromaSampling::Monochrome => {
                        return Err(Error::Meta(yuv::Error::InvalidDepthRequested));
                    }
                };
            ...
            gray_iters => {
                let mc = img
                    .matrix_coefficients()
                    .unwrap_or(color::MatrixCoefficients::Identity);
    ...
    fn to_gray(
        range: aom_decode::color::Range,
        mc: color::MatrixCoefficients,
        iters: RowsIters,
    ) -> Result<AlphaImage, yuv::Error> {
        Ok(match iters {
            RowsIters::YuvPlanes8 { y, .. } | RowsIters::Mono8(y) => {
                ...
                out.extend(y.flat_map(|row| {
                    row.iter()
                        .copied()
                        .map(|y| Gray::new(conv.to_rgb(YUV { y, u: 128, v: 128 }).g))
                }));
